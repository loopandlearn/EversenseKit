...
enum DMSApi {
    ...
    static func uploadGlucoseReadings(cgmManager: EversenseCGMManager, readings: [CGMReadingResult]) async -> Bool {
    ...
    static func uploadCurrentValues(cgmManager: EversenseCGMManager, reading: CGMReadingResult) async -> Bool {
    ...
    static func uploadDeviceEvents(cgmManager: EversenseCGMManager, readings: [CGMReadingResult]) async -> Bool {
    ...
    private static func getAccessToken(cgmManager: EversenseCGMManager) async -> String? {
        var accessToken = cgmManager.state.accessToken
        let expiration = cgmManager.state.accessTokenExpiration
        
        if let accessToken, let expiration, Date.now.timeIntervalSince(expiration) > 0 {
            return accessToken
        }
    ...
    private static func buildSgBytes(readings: [CGMReadingResult], sensorId: String) -> String {
